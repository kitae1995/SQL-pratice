...
트리거는 그 자체를 트랜잭션의 일부로 처리하기 떄문에
commit이나 rollback을 포함 할 수 없음
*/
...
CREATE TABLE tbl_user_backup
    ( id VARCHAR2(20) PRIMARY KEY,
      name VARCHAR2(20),
      addrees VARCHAR2(30),
      update_date DATE DEFAULT sysdate, -- 정보 변경 시간 ( insert 되는 시간 )
      m_type VARCHAR2(10), -- 변경타입
...
BEGIN
    ...
    IF UPDATING THEN -- UPDATING은 시스템 자체에서 상태에 대한 내용을 지원하는 빌트인 구문 ( 업데이트 된다면? 이라는 뜻)
        v_type := '수정';    
    ELSIF DELETING THEN 
        v_type := '삭제';
    END IF;
    
    -- 본격적인 실행 구문 작성 (backup 테이블에 값을 insert -> 원본 테이블에서 UPDATE OR DELETE된 사용자 정보)
    INSERT INTO tbl_user_backup -- 백업테이블이니 예전 데이터가 들어가야함
    VALUES(:OLD.id,:OLD.name,:OLD.address,sysdate,v_type,USER()); -- USER() 현재 사용자의 계정명
...
DECLARE
    ...
    quantity_shortage_exception EXCEPTION; -- 수량부족할시 예외처리할 EXCEPTION 타입
...
BEGIN
    ...
    v_product_total
    FROM product
    WHERE product_no = v_product_no; -- 상품 번호 확인
   

    
   
    IF v_product_total <= 0 THEN -- 사용자가 주문하려는 양이 재고보다 많다면?
    ...
    EXCEPTION 
        ...
        -- 두번째 매개값 : 에러 메세지
        RAISE_APPLICATION_ERROR(-20001,'주문하신 수량보다 재고가 적어서 주문이 안됩니다');
        WHEN zero_total_exception THEN
        RAISE_APPLICATION_ERROR(-20002,'주문하신 상품의 재고가 없어 주문 할 수 없습니다.');
END;
...
--트리거 내에서 예외가 발생하면 수행중인 INSERT 작업은 중단되며 rollbakc이 진행됨.
INSERT INTO order_history VALUES(order_history_seq.NEXTVAL,201,2,100,20000);